...
def find_indexPrice(row):
    ...
    index_Price = yf.download('NIFTYBEES.NS', start, end)[
...
def print_sortedDF(df_holding):
    ...
    df_holding = df_holding.sort_values(
        by=['ThreeDay_Change'], ascending=False)
    ...
    df_holding = df_holding.sort_values(
        by=['SevenDay_Change'], ascending=False)
    ...
    df_holding = df_holding.sort_values(
        by=['FourteenDay_Change'], ascending=False)
    ...
    df_holding = df_holding.sort_values(
        by=['ThirtyDay_Change'], ascending=False)
